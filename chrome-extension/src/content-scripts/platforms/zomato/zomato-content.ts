/**
 * Zomato Content Script - Platform-specific implementation
 */
...
import { ZomatoPageDetector } from './zomato-page-detector';
...
/**
 * Zomato-specific content script implementation
 */
export class ZomatoContentScript implements IPlatformContentScript {
  public readonly platform = Platform.ZOMATO;
  private readonly logger = console;
  private readonly config: PlatformConfig;
  private readonly pageDetector: ZomatoPageDetector;
  ...
  constructor() {
    this.config = ZOMATO_CONFIG;
    this.pageDetector = new ZomatoPageDetector();
  ...
  public async initialize(): Promise<void> {
    try {
      if (this.isInitialized) {
        this.logger.warn('[Zomato] Content script already initialized');
        return;
      }

      this.logger.log('🚀 [Zomato] Initializing content script...');
      ...
      this.logger.log('✅ [Zomato] Content script initialized');
    } catch (error) {
      this.logger.error('[Zomato] Failed to initialize content script:', error);
      throw error;
    }
  }
  ...
  public async handleMessage(message: ExtensionMessage): Promise<MessageResponse> {
    try {
      this.logger.log('[Zomato] Received message:', message.type, message.payload);
      ...
      this.logger.error('[Zomato] Error handling message:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      };
  ...
  private async handleStartOrder(payload: unknown): Promise<MessageResponse> {
    this.logger.log('[Zomato] Starting order automation with payload:', payload);
  ...
  private async handleExecuteAction(instruction: ActionInstruction): Promise<MessageResponse> {
    this.logger.log('[Zomato] Executing action:', instruction.action);
  ...
  private injectStyles(): void {
    ...
    style.textContent = `
      .foodbot-highlight {
        outline: 2px solid #e23744 !important;
        outline-offset: 2px !important;
        animation: foodbot-pulse-zomato 1s infinite;
      }

      @keyframes foodbot-pulse-zomato {
        0%, 100% {
          outline-color: #e23744;
        }
        50% {
